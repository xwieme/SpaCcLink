...
def getInteractionDB(
    genes_use,
    species,
    lr_path=f"{ROOT_DIR}/prior_db/lr_pair.csv",
    pathways_path=f"{ROOT_DIR}/prior_db/pathways.csv",
    tftg_path=f"{ROOT_DIR}/prior_db/TFTG.csv",
    rec_tf_path=f"{ROOT_DIR}/prior_db/rec_tf.csv",
):
    ...
    pathways = pd.concat([pathways, tftg], ignore_index=True).drop_duplicates(
        keep=False
    )
