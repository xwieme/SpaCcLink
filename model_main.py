...
def fit_one_epoch(model, train_loader, test_loader, data, optimizer, epoch, Epoch, n_train_batch, n_test_batch,
                  device, loss_file, save_period = 5):
    ...
    print('Epoch:' + str(epoch + 1) + '/' + str(Epoch)+ "\t\t"+ 'Train Loss: %.3f  Test Loss: %.3f ' % (train_loss, test_loss))
...
def train(config=None):
    ...
    mapping = dict(zip(express_matrix.columns.values, range(len(express_matrix.columns.values))))
    ...
    for epoch in tqdm(range(Epoch)):
        ...
        fit_one_epoch(model, train_loader, test_loader, data, optimizer, epoch, Epoch, n_train_batch, n_test_batch, device, loss_file, save_period)
...
def test(config=None):
    ...
    neg = torch.cat((neg, torch.zeros(neg.shape[0], dtype=torch.int).view(-1, 1)), dim=1)
...
def predict():
    ...
    pred_loader = DataLoader(pred_dataset, batch_size=batch_size, num_workers=num_workers, drop_last=False,
                             shuffle=False)
    ...
    top_pairs.to_csv(os.path.join(cp.get("data", "output_dir"), "top_pairs.csv"), index=False)
